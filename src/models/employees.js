...
export class EmployeeModel {
  ...
  static async createEmployee(data, uuid, userId) {
    const { first_name, middle_name, last_name, dni, email, phone, address, salary } = data
    const [employeeResult] = await pool.query(`INSERT INTO employees (id_employee,first_name, middle_name, last_name, dni, email, phone, address, salary, user_id) VALUES (UUID_TO_BIN("${uuid}"),?, ?, ?, ?, ?, ?, ?, ?, UUID_TO_BIN("${uuid}"))`,
      [first_name, middle_name, last_name, dni, email, phone, address, parseFloat(salary), userId]);
